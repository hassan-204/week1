...
template sudoku() {
    ...
    component rangeProofPuz = RangeProof(32);
    component rangeProofSol = RangeProof(32);
    component comp[162];
    
    rangeProofPuz.range[0] <== 0;
    rangeProofPuz.range[1] <== 10;

    rangeProofSol.range[0] <== 0;
    rangeProofSol.range[1] <== 10;    

var x=0;
while (x<81){
    for (var i=0; i<9; i++) {
        for (var j=0; j<9; j++) {
            comp[x] = RangeProof(32);
            comp[x+81] = RangeProof(32);   

            comp[x].in <== puzzle[i][j];
            comp[x+81].in <== solution[i][j];
